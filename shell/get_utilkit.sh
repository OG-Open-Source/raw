...
if [ -f ~/utilkit.sh ]; then
	...
	if [[ $version == 0.* ]]; then
		if [ "$dis_lang" = "z1" ]; then
			echo -e "${CLR3}警告：此版本（$version）為開發版本。不建議使用${CLR0}"
			echo -e "${CLR3}建議等待正式版本重新發布後再下載${CLR0}"
			read -p "是否仍然下載使用？(y/N) " -n 1 -r
			echo
			[[ ! $REPLY =~ ^[Yy]$ ]] && { echo -e "${CLR3}下載已取消${CLR0}"; exit; }
			...
			echo
			[[ ! $REPLY =~ ^[Yy]$ ]] && { echo -e "${CLR3}下载已取消。${CLR0}"; exit; }
		else
			...
			echo
			[[ ! $REPLY =~ ^[Yy]$ ]] && { echo -e "${CLR3}Download cancelled.${CLR0}"; exit; }
		fi
	fi
